...
#%%计算修改后的降水量平均态
#主3mm/day等值线以西仍有部分格点降水量大于3mm/day，把那些格点的降水量修改至3以下以便画等值线图
pre_xiugai=pre_mean.copy() #用来画3mm/day等值线的数组
I,J=pre_mean.shape
for i in range(I):
    for j in range(J):
        if Lon[i,j]<103 and Lat[i,j]>36 and pre_xiugai[i,j]>=3:
            pre_xiugai[i,j]=2.99999
        elif Lon[i,j]>88 and Lat[i,j]<30.1 and pre_xiugai[i,j]<=3:
            pre_xiugai[i,j]=3.00001
        elif Lon[i,j]>118.1 and Lat[i,j]<43.8 and pre_xiugai[i,j]<=3:
            pre_xiugai[i,j]=3.00001
        elif Lon[i,j]>119.5 and Lat[i,j]<45.5 and pre_xiugai[i,j]<=3:
            pre_xiugai[i,j]=3.00001
        elif Lon[i,j]>111 and Lat[i,j]<38 and pre_xiugai[i,j]<=3:
            pre_xiugai[i,j]=3.00001
        elif Lon[i,j]>106.8 and Lat[i,j]<35.9 and pre_xiugai[i,j]<=3:
            pre_xiugai[i,j]=3.00001
...
c2=ax1.contour(lon,lat,pre_xiugai,[3],transform=ccrs.PlateCarree(),colors='r',linewidths=1.5)
...
ax1=plt.subplot(111,projection=ccrs.PlateCarree())
ax1.set_extent([72,137,17,55],crs=ccrs.PlateCarree())
ax1.add_geometries(CNTPgdf.geometry,crs=ccrs.PlateCarree(),edgecolor='none',linewidths=0,facecolor='grey')
ax1.add_geometries(shp.geometries(),crs=ccrs.PlateCarree(),edgecolor='k',linewidths=1.2,facecolor='none')
ax1.add_geometries(TPgdf.geometry,crs=ccrs.PlateCarree(),edgecolor='b',linewidths=1.2,facecolor='none')
ax1.add_geometries(counter_line_gdf0.geometry,crs=ccrs.PlateCarree(),edgecolor='r',linewidths=1.2,facecolor='none')
...
ax1=plt.subplot(111,projection=ccrs.PlateCarree())
ax1.set_extent([72,137,17,55],crs=ccrs.PlateCarree())
ax1.add_geometries(NWCgdf.geometry,crs=ccrs.PlateCarree(),edgecolor='b',linewidths=1.2,facecolor='none')
ax1.add_geometries(NWC_TPgdf.geometry,crs=ccrs.PlateCarree(),edgecolor='green',linewidths=1.2,facecolor='none')
...
ax1.add_feature(cfeature.OCEAN,color='lightblue')
cf=ax1.contourf(lon,lat,pre_mean,lines,transform=ccrs.PlateCarree(),cmap=cmap,extend='max')
c1=ax1.contour(lon,lat,pre_xiugai,[3],transform=ccrs.PlateCarree(),colors='r',linewidths=0.5)
c2=ax1.contour(lon,lat,pre_xiugai,[3],transform=ccrs.PlateCarree(),colors='r',linewidths=1.5)
